...
# Memanggil Database ke View User

### Tanpa Perulangan

1. Memanggil Model di File web.php
   Untuk memulai, tambahkan model Jumbotron ke file web.php. Kemudian, gunakan metode first() untuk mengambil data pertama dari tabel jumbotron (dalam kasus ini, data dengan id paling kecil). Berikut contohnya:

    ```bash
    use App\Models\Jumbotron;

    Route::get('/', function () {
        $jumbotron = Jumbotron::first(); // Mengambil data pertama dari tabel Jumbotron
        return view('user.beranda.beranda', compact('jumbotron')); // Mengirim data ke view
    })->name('beranda');
    ```

2. Memanggil Data Tabel ke File View
   Di file view (misalnya beranda.blade.php), gunakan double curly braces {{ }} untuk menampilkan data dari tabel. Misalnya, Anda ingin menampilkan gambar, judul, dan deskripsi dari tabel jumbotron:

    ```bash
    <div style="background-image: url('{{ asset('storage/' . $jumbotron->gambar) }}'); background-size: cover; background-position: bottom; padding-top: 310px; padding-bottom: 310px;">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <h1 class="text-[72px] font-bold text-white w-[60%] leading-[80px] mb-4">{{ $jumbotron->judul }}</h1>
            <p class="text-base font-medium text-white w-[60%] leading-[24px]">{{ $jumbotron->deskripsi }}</p>
        </div>
    </div>
    ```
    ...
    {{ $jumbotron->gambar }} digunakan untuk menampilkan kolom gambar.
    {{ $jumbotron->judul }} digunakan untuk menampilkan kolom judul.
    {{ $jumbotron->deskripsi }} digunakan untuk menampilkan kolom deskripsi.

### Menggunakan Perulangan

1. Memanggil Model di File web.php
   Untuk memuat semua data dari tabel Promo_kami, gunakan metode all() dalam file web.php. Selain itu, Anda juga bisa memuat data dari model lain (seperti Jumbotron).
    ...
    Route::get('/', function () {
        $jumbotron = Jumbotron::first(); // Mengambil data pertama dari tabel Jumbotron
        $promos = Promo_kami::all(); // Mengambil semua data dari tabel Promo_kami
        return view('user.beranda.beranda', compact('jumbotron', 'promos')); // Mengirim data ke view
    })->name('beranda');
    ```

2. Melakukan Perulangan di File View
   Gunakan perulangan @foreach untuk menampilkan semua data dari tabel Promo_kami di file view:
    ...
    @foreach digunakan untuk mengulangi setiap data yang ada di variabel $promos.
    {{ $promo->gambar }} akan menampilkan gambar dari kolom gambar di tabel Promo_kami.
